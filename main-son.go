...
func handler(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Println(JobType + JobName + JobId)

	db, err := sql.Open("mysql", "togepi:1@tcp(127.0.0.1:3306)/redisdb")
	if err != nil {
		panic(err)
	}
	sqlQ := "SELECT * FROM gearman_jobs WHERE %s ORDER BY `Date` DESC"
	whereQuery := "1=1"
	var rows *sql.Rows
	customWhere := []string{}
	if len(JobType) > 0 {
		customWhere = append(customWhere, fmt.Sprintf("`JobType` = '%s'", JobType))
	}

	if len(JobName) > 0 {
		customWhere = append(customWhere, fmt.Sprintf("`JobName` = '%s'", JobName))
	}
	if len(JobId) > 0 {
		customWhere = append(customWhere, fmt.Sprintf("`JobId` = '%s'", JobId))
	}
	if(len(customWhere) > 0) {
		whereQuery = strings.Join(customWhere, " AND ")
	}
	rows, err = db.Query(fmt.Sprintf(sqlQ, whereQuery))
